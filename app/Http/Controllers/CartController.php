...
use App\Models\Transaction;                     // ← THIS WAS MISSING!
...
class CartController extends Controller
{
    public function index()
    {
        ...
        return view('home.cart', compact('items'));
    }
    ...
    public function apply_coupon_code(Request $request)
    {
        ...
        {
        // If no coupon → remove old discount data
        ...
        // THIS IS THE CRITICAL FIX: convert the formatted string (e.g. "1,299.00") to float
        $subtotal = (float) str_replace(',', '', Cart::instance('cart')->subtotal());

        // Calculate discount amount
        if ($coupon['type'] === 'fixed') {
            $discount = (float) $coupon['value'];                    // e.g. 50.00
        } else {
            $discount = ($subtotal * (float) $coupon['value']) / 100; // percentage
        }

        $subtotalAfterDiscount = $subtotal - $discount;
        $tax                    = ($subtotalAfterDiscount * config('cart.tax')) / 100;
        $total                  = $subtotalAfterDiscount + $tax;

        // Store clean, formatted values in session
    ...
    public function place_an_order(Request $request)
    {
        ...
        if(!$address)
        {
            ...
            $address->landmark = $request->landark;
        ...
        foreach(Cart::instance('cart')->content() as $item)
        {
            ...
            $orderItem->product_id = $item->model->id;  // This is almost certainly what you need
            // Correct order ID
