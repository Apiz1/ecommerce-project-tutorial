...
class AdminController extends Controller
{
    ...
    public function brand_store(Request $request)
    {
        ...
        if($image)
        {
            ...
            $request->image->move('brand_image',$imagename);
    ...
    public function brand_update(Request $request)
{
    $request->validate([
        'name' => 'required',
        'slug' => 'required|unique:brands,slug,' . $request->id, // allow current brand's slug
    ...
    if ($request->hasFile('image')) {
        ...
        $oldImagePath = public_path('brand_image/' . $brand->image);
        ...
        $imagename = time() . '.' . $image->getClientOriginalExtension();
        $image->move(public_path('brand_image'), $imagename);
        $brand->image = $imagename;
    }
...
{
    $brand = Brand::find($id);
    if(File::exists(public_path('brand_image').'/'.$brand->image))
    {
        File::delete(public_path('brand_image').'/'.$brand->image);
    }
    $brand->delete();
...
public function category_update(Request $request)
{
    $request->validate([
        'name' => 'required',
        'slug' => 'required|unique:categories,slug,' . $request->id, // allow current brand's slug
        'image'=> 'nullable|mimes:png,jpg,jpeg|max:2048'
    ]);
    ...
    if ($request->hasFile('image')) {
        // Delete old image if it exists
        $oldImagePath = public_path('category_image/' . $category->image);
...
public function product_store(Request $request)
{
    ...
    if ($request->hasFile('images')) {
        ...
        foreach ($request->file('images') as $file) {
            ...
            if (in_array($gextension, $allowedFileExt)) {
                $gfileName = $current_timestamp . '-' . $counter . '.' . $gextension;
                $file->move(public_path('product_gallery'), $gfileName);
...
public function product_update(Request $request)
{
    ...
    // ✅ Find existing product instead of creating new
    ...
    // ✅ Handle main image upload with delete/replace
    if ($request->hasFile('image')) {
        // Delete old image if exists
        if (!empty($product->image)) {
            $oldImagePath = public_path('product_image/' . $product->image);
    ...
    // ✅ Handle gallery images upload (replace old ones)
    if ($request->hasFile('images')) {
        $allowedFileExt = ['jpg', 'png', 'jpeg'];
        $gallery_arr = [];
        $counter = 1;
        ...
        // Delete old gallery images if stored
        if (!empty($product->images)) {
            foreach (explode(',', $product->images) as $oldGalleryImage) {
                $oldGalleryPath = public_path('product_gallery/' . $oldGalleryImage);
...
public function product_delete($id)
{
    ...
    if (!empty($product->image)) {
        $imagePath = public_path('product_image/' . $product->image);
    ...
    if (!empty($product->images)) {
        foreach (explode(',', $product->images) as $galleryImage) {
            $galleryPath = public_path('product_gallery/' . $galleryImage);
    ...
    public function coupon_update(Request $request)
    {
        ...
        $coupon->expiry_date = $request->expiry_date;
        $coupon->save();
        return redirect()->route('admin.coupons')->with('status','Coupon has been update succesfully');
    }
   ...
   public function update_order_status(Request $request)
{
    ...
    if ($request->order_status == 'delivered') {
        ...
        $order->canceled_date = Carbon::now();  // ← Fixed: added "ed"
    }

    $order->save();

    // Update transaction status only when delivered
    if ($request->order_status == 'delivered') {
    ...
    public function slide_add()
    {
        ...
        public function slide_store(Request $request)
        {
            ...
            {
                $imagename = time().'.'.$image->getClientOriginalExtension();
                $request->image->move('slide_image',$imagename);
        ...
        public function slide_update(Request $request)
        {
            ...
            if ($request->hasFile('image')) {
            // Delete old image if it exists
            $oldImagePath = public_path('slide_image/' . $slide->image);
            if ($slide->image && file_exists($oldImagePath)) {
                unlink($oldImagePath);
            }
            ...
            $image = $request->file('image');
            $imagename = time() . '.' . $image->getClientOriginalExtension();
            $image->move(public_path('slide_image'), $imagename);
            $slide->image = $imagename;
        }
            $slide->save();
            return redirect()->route('admin.slides')->with('status','Slide has been update successfully');
        }

        public function slide_delete($id)
        {
            $slide = Slide::find($id);
            if(File::exists(public_path('slide_image').'/'.$slide->image))
                {
                    File::delete(public_path('slide_image').'/'.$slide->image);
                }
            $slide->delete();
